 ...
 */
	...
	public function report_outputs($match = null, $ba_only = false, $proj_only = false, $no_sum = false)
	{
		...
		$where1 = 
			"(
			 ((NOT (`Bizassist`.`id` IS NULL)) AND (`Bizassist`.`enabled` = '1') AND (`BComp`.`enabled` = '1'))
				OR 
			 ((NOT (`Project`.`id` IS NULL)) AND (`Project`.`enabled` = '1') AND (`PComp`.`enabled` = '1'))
			 )";
			 
		$joins1 = "
			LEFT JOIN `rickdbprojects`.`bizassists` AS `Bizassist` ON (`Output`.`bizassist_id` = `Bizassist`.`id`) 
			LEFT JOIN `rickdbprojects`.`companies` AS `BComp` ON (`Bizassist`.`company_id` = `BComp`.`id`) 
			LEFT JOIN `rickdbprojects`.`projects` AS `Project` ON (`Output`.`project_id` = `Project`.`id`) 
			LEFT JOIN `rickdbprojects`.`companies` AS `PComp` ON (`Project`.`company_id` = `PComp`.`id`) 
			";
			
	  	if ($match) {
			$cond_match = "AND (`Opstatustype`.`title` = $match)";
		} else {
			$cond_match = "";
		}
					
		if ($ba_only == true) {
			$where1 = "((NOT (`Bizassist`.`id` IS NULL)) AND (`Bizassist`.`enabled` = '1') AND (`BComp`.`enabled` = '1'))";
			$joins1 = "
			LEFT JOIN `rickdbprojects`.`bizassists` AS `Bizassist` ON (`Output`.`bizassist_id` = `Bizassist`.`id`) 
			LEFT JOIN `rickdbprojects`.`companies` AS `BComp` ON (`Bizassist`.`company_id` = `BComp`.`id`) 
			";
		...
		if ($proj_only == true) {
			$where1 = "((NOT (`Project`.`id` IS NULL)) AND (`Project`.`enabled` = '1') AND (`PComp`.`enabled` = '1'))";
			$joins1 = "
			LEFT JOIN `rickdbprojects`.`projects` AS `Project` ON (`Output`.`project_id` = `Project`.`id`) 
			LEFT JOIN `rickdbprojects`.`companies` AS `PComp` ON (`Project`.`company_id` = `PComp`.`id`) 
			";
		...
		$query = 
		"SELECT `Optype`.`title` AS `Type`, `Opstatustype`.`title` AS `Status`, SUM(`Output`.`quantity`) As Output__Quantity
		FROM `rickdbprojects`.`outputs` AS `Output` 
		LEFT JOIN `rickdbprojects`.`optypes` AS `Optype` ON (`Output`.`optype_id` = `Optype`.`id`) 
		LEFT JOIN `rickdbprojects`.`opstatustypes` AS `Opstatustype` ON (`Output`.`opstatustype_id` = `Opstatustype`.`id`) 
		$joins1
		WHERE 
		(
			$where1
			AND (`Output`.`enabled` = '1')
			$cond_match
		)
		GROUP BY optype_id, opstatustype_id 
		ORDER BY `opstatustype_id` ASC, `optype_id` ASC";
		
			  	$this->Output->virtualFields['Quantity'] = 0;
	  	$this->set('outputs', $this->Output->query($query));
	  	
	  	/*
	  	$find_conditions = array(
	  				//'recursive' => 1,
	  				'fields' => array('Optype.title AS Type', 'Opstatustype.title AS Status', 'SUM(Output.quantity) As Quantity', '*', 'Bizassist.*', 'Project.*', 'Project.company_id'
	  				//, 'Company.id', 'Company.*'
	  				),
	  				'order' => array('opstatustype_id', 'optype_id')
	  				,'group' => array('optype_id', 'opstatustype_id')
	  				,'contain' => array('Bizassist', 'Project' => array('fields' => array('id', 'enabled', 'company_id'), 'Company' => array('fields' => array('id', 'enabled'))), 'Optype', 'Opstatustype')
	  				//,
	  				//'joins' => array('table' => 'companies', 'alias' => 'Company', 'conditions' => array('Company.id = Project.company_id'))
	  				);

  		//$find_conditions['conditions']['Output.enabled'] = true;
  		//$find_conditions['conditions']['Bizassist.enabled'] = true;
  		//$find_conditions['conditions']['Project.enabled'] = true;
  		//$find_conditions['conditions']['Company.enabled'] = true;
		...
		//$this->Output->contain('Bizassist', 'Project' => array('Company'), 'Optype', 'Opstatustype');
		//$this->Output->Project->contain('Company');
	  	//$this->Output->Behaviors->load('Containable', array('autoFields' => false));
	  	//$this->Output->Behaviors->load('Containable', array('recursive' => true, 'autoFields' => false));
		$outputs = $this->Output->find('enabled', $find_conditions);
		//$outputs = $this->Output->find('all', $find_conditions);
	...
	{
		$this->report_outputs($match, true, false);
	}
	...
	public function report_all($match = null) {
		$this->report_outputs($match, false, false);
	}
